...
let ps4_goldhen = {
    ...
    request( onSuccess=null, onResponse=null ){
        return new Promise( (resolve, reject) => {
            ...
            const handleTimeout = () => {
                console.error("PS4 API connection timed out");
                reject("PS4 Connection timed out. PS4 not available on " + this.getURL('url') )
                socket.destroy()
            };
    ...
    send(requestObject={}){
        return new Promise((resolve, reject) => {        
            ...
            const timeoutId = setTimeout(() => {
                client.destroy()
                reject( new Error('PS4 Connection timed out at ' + this.getURL('url') ) )
            }, this.getTimeout());
            ...
            client.on('error', (error) => {
                ...
                if( error.code == 'ECONNREFUSED' )
                    return reject(`PS4 Connection failed at ${this.getURL('url')}`)
            ...
            client.on('close', () => {
                console.log("Close Connection to PS4")
                clearTimeout(timeoutId);
            })
        })
    },
